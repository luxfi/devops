...
The **lux-operator** is deployed and managing all 3 networks on `do-sfo3-lux-k8s`:

- **CRDs**: `LuxNetwork` + `LuxChain` + `LuxIndexer` + `LuxExplorer` + `LuxGateway` (`lux.network/v1alpha1`)
- **Namespace**: `lux-system` (operator runs here)
- **Image**: `registry.digitalocean.com/hanzo/lux-operator:v0.3.4`
- **Image Pull Secret**: `registry-hanzo` (DO container registry)
- **Status**: All 3 networks `Running` with 5/5 validators each (adopted existing)
- **Reconcile loop**: 60s interval, health checks, status reporting
- **Snapshots**: Hourly automated snapshots to MinIO (`lux-snapshots` bucket), configurable per network

Deploy manifests: `k8s/operator/` (CRDs, RBAC, Deployment, Service, Network CRs)
Deploy script: `k8s/operator/deploy.sh`

Key architecture: `LuxNetwork` CR → operator reconciles → adopts/manages StatefulSets + Services.

### Snapshot Infrastructure (2026-02-19)

Automated snapshot system for fast node bootstrap and disaster recovery:

- **Storage**: MinIO S3-compatible at `minio.hanzo.svc.cluster.local:9000`
- **Bucket**: `lux-snapshots` with anonymous download policy
- **Credentials**: `hanzo` / `hanzo-minio-secret`
- **Schedule**: Hourly via operator reconcile loop (configurable `intervalSeconds`)
- **Flow**: `admin.snapshot` RPC → `admin_exportChain` RLP → `tar czf` → `mc cp` to MinIO
- **Restore**: Init container downloads `.tar.gz` from MinIO on first boot (`.seeded` marker)
- **Contents**: `db/`, `chainData/`, `genesis.bytes` per snapshot archive

Snapshot CRD fields on `LuxNetwork.spec.snapshotSchedule`:
```yaml
snapshotSchedule:
  enabled: true
...
Status fields: `lastSnapshotTime`, `lastSnapshotUrl`, `lastSnapshotVersion`, `snapshotCount`.

### KMS Secret Management (2026-02-19)

All Lux infrastructure secrets flow through **Hanzo KMS** (`kms.lux.network`):

- **KMS Operator**: `kms-operator-system` namespace, image `ghcr.io/luxfi/kms-operator:latest`
- **CRD**: `KMSSecret` (`secrets.lux.network/v1alpha1`) → syncs KMS secrets to K8s Secrets
- **Auth**: `universal-auth-credentials` secret in `hanzo` namespace (shared cluster-wide)
- **Identity**: `k8s-operator` (universal machine identity, admin role)
- **KMS Project**: `lux-infra` (slug), environment: `prod`

KMSSecret resources:
| Namespace | Name | KMS Path | Target Secret | Type |
|-----------|------|----------|---------------|------|
| lux-system | ghcr-pull-secret | /ghcr | ghcr-luxfi | dockerconfigjson |
| lux-system | do-registry-secret | /registry | registry-hanzo | dockerconfigjson |
| lux-mainnet | do-registry-secret | /registry | registry-hanzo | dockerconfigjson |
| lux-testnet | do-registry-secret | /registry | registry-hanzo | dockerconfigjson |
| lux-devnet | do-registry-secret | /registry | registry-hanzo | dockerconfigjson |

**Critical RBAC requirement**: The KMS operator's ClusterRole (`kms-operator-manager-role`) must include `daemonsets` and `statefulsets` in the `apps` API group alongside `deployments`. The auto-redeploy feature registers informers for all workload types, and missing RBAC causes a reflector error loop that **blocks reconciliation of all KMSSecrets after the first one**. This was the root cause of lux-* KMSSecrets not syncing.
...
```
registry.digitalocean.com/hanzo/bootnode:luxd-v1.23.15-debug
```
...
### CRDs (5 total — consolidated 2026-02-19)

| CRD | Purpose | K8s Resources Created |
|-----|---------|----------------------|
| `LuxNetwork` | Manages a full validator network (mainnet/testnet/devnet) | StatefulSet, Services (headless + per-pod LB), ConfigMaps, RBAC, PDB, KMSSecrets |
| `LuxChain` | Manages individual subnet chain deployments, links to parent LuxNetwork | Updates parent network's chain tracking config |
| `LuxIndexer` | Deploys blockchain indexer for one EVM chain (C-chain or subnet) | Deployment + PostgreSQL StatefulSet + Service |
| `LuxExplorer` | Deploys block explorer web UI (Blockscout-based), connects to LuxIndexers | Deployment + Service + Ingress |
| `LuxGateway` | Deploys API gateway (KrakenD), routes RPC/indexer/explorer traffic | Deployment + ConfigMap (KrakenD config) + Service + Ingress |

### White-Label Architecture (2026-02-19)

Lux services are **white-labeled Hanzo services** using Lux org in Hanzo IAM:

- **Same operator codebase** runs for both Hanzo and Lux ecosystems
- **Standalone repo**: `github.com/luxfi/operator` (GHCR image: `ghcr.io/luxfi/operator:v0.3.0`)
- **DevOps workspace**: `lux-operator/` member of `lux-ops` workspace (DO image: `registry.digitalocean.com/hanzo/lux-operator`)
- **KMS project mapping**: `lux-infra` project in Hanzo KMS provides Lux-specific secrets
- **Branding**: Explorer/Gateway CRDs have branding fields (`networkName`, `logoUrl`, `primaryColor`, `host`)
- **Image sources**: Indexer/Explorer/Gateway images can reference either Hanzo or Lux registries
- **IAM boundary**: Hanzo IAM org isolation controls which KMS projects, registries, and domains each org accesses

The operator itself is org-agnostic — multi-tenancy is achieved through:
1. Namespace isolation (e.g., `lux-mainnet` vs `hanzo-mainnet`)
2. KMS project isolation (e.g., `lux-infra` vs `hanzo-infra`)
3. Per-CR branding and host configuration
4. Image pull secret isolation (GHCR vs DO registry per deployment)
