//! Staking Signer Key Downloader
//!
//! Downloads BLS staking signer key from S3 with KMS decryption.
...
pub const APP_NAME: &str = "staking-signer-downloader";

const GZIP_MAGIC: [u8; 2] = [0x1f, 0x8b];

#[derive(Parser)]
#[command(name = APP_NAME)]
#[command(about = "Download staking BLS signer key from S3")]
#[command(version)]
struct Cli {
    /// Log level
...
async fn main() -> Result<()> {
    ...
    info!("S3: s3://{}/{}", cli.s3_bucket, cli.s3_key);
    info!("Output: {}", cli.key_path);
